...
const WeatherCard: React.FC<WeatherCardProps> = ({
  ...
  return <div className="glass rounded-2xl p-4 sm:p-6 slide-up hover-glow">
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mb-4 sm:mb-6">
        <input type="text" value={locationInput} onChange={e => setLocationInput(e.target.value)} onKeyPress={handleKeyPress} placeholder="Enter Location" className="flex-1 px-3 py-2.5 sm:px-4 sm:py-3 bg-white/20 border border-white/30 rounded-xl text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-white/50 focus:border-white/50 backdrop-blur-sm text-sm sm:text-base transition-all duration-300" />
        <button onClick={searchWeather} className="px-4 py-2.5 sm:px-6 sm:py-3 bg-white/25 hover:bg-white/35 text-white rounded-xl transition-all duration-300 font-medium backdrop-blur-sm border border-white/20 hover:border-white/40 text-sm sm:text-base min-w-[60px] sm:min-w-[80px] flex items-center justify-center">
          🔍
        </button>
      </div>

      {/* Error Message */}
      {error && <div className="bg-red-500/20 border border-red-400/30 rounded-xl p-3 mb-4 backdrop-blur-sm">
      ...
      {weather && <div className="space-y-4 sm:space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
            <div className="flex-1">
              ...
              <p className="text-white/80 capitalize text-sm sm:text-base text-center">{weather.condition}</p>
            </div>
